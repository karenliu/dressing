...
\karen{My understanding of the system is closer to the following
  modification to Figure 1. Change ``action queue'' box to ``action planner.  Action
  queue is the output data from action planner. The dressing
  controller looks up this queue at every time step and decides which
  controller to activate. The feedback arrow should go into dressing
  controller, not action queue.}
