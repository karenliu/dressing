...
We have designed a system that allows a virtual human character to put
...
of high level actions \karen{What's the relation between this sequence
  of actions and the reference motion? We might want to clarify that
  here.}. We call these actions \emph{primitive actions}, which we
designed specifically for dressing \karen{We also want to stress that
  a primitive action is a parameterizable building block.}\jie{Greg suggests that primitive action and high level action have opposite meanings.} For example, putting an arm into a sleeve can be described as first \emph{aligning} the hand with the armhole and then \emph{dragging} the cloth from the wrist to the shoulder. Table~\ref{table:actionQueue} shows a complete action queue for putting on a jacket. At each step, our system fetches an action from the action queue, executes its corresponding dressing controllers and simulates the physics of the cloth. Figure \ref{fig:overview} illustrates the main components of our system. 
